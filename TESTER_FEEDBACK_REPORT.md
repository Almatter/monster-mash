...
- Basic attacks now wait until a valid enemy is in range. Devourer's claw arcs mark the outer attack zone rather than covering its sprite, letting the champion and hit reactions show. Ranged touch auto-attacks select a close, meaningful threat from the spatial grid, retain a useful target, and switch for an urgent near threat. Explicit mouse aiming still works on desktop. There is no full-enemy sort each frame.
...
- Procedural repeated attacks now rotate through related seeded waveform variations with subtle pitch/playback-rate drift. Devourer's basic has six variants; other basics have four. Slow noise modulation adds organic texture without changing each monster's sound identity.
