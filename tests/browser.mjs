...
await page.locator('#name').fill('Test Sovereign');await page.locator('#startForm button').click();
...
await page.locator('#endRun').click();await page.waitForFunction(()=>document.querySelector('#runCode').value.startsWith('MM1.'));
...
const mobile=await browser.newContext({viewport:{width:844,height:390},isMobile:true,hasTouch:true,deviceScaleFactor:2});const touch=await mobile.newPage();touch.on('pageerror',e=>errors.push(e.message));await touch.goto('http://127.0.0.1:4173');await touch.locator('#startForm button').tap();
