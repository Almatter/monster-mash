...
export class Game {
 ...
 moving=false;momentum=0;momentumGrace=0;lungeCharges=1;lungeRecharge=0;dodgeInvuln=0;dodgeDeflected=false;resonance=0;basicTarget:Enemy|null=null;basicTargetSerial=0;targetPoint:{x:number;y:number}|null=null;servants=createServants();corruptionTick=0;chains:{x:number;y:number}[]=[];
 ...
 update(dt:number,input:Input){
  ...
  const startX=p.x,startY=p.y;this.moving=!!(input.x||input.y||this.dash);const length=Math.hypot(input.x,input.y)||1;p.x+=input.x/Math.max(1,length)*this.monster.speed*this.releaseStats.move*(this.monster.passive.id==='hunger'?1+Math.min(.15,this.score.recent.length*.003):1)*dt;p.y+=input.y/Math.max(1,length)*this.monster.speed*this.releaseStats.move*(this.monster.passive.id==='hunger'?1+Math.min(.15,this.score.recent.length*.003):1)*dt;
