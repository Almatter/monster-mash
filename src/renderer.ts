...
export class Renderer {
 ...
 draw(g:Game|null,time:number){
  ...
  for(let i=0;i<8;i++){const a=i*Math.PI/4,x=Math.cos(a)*580,y=Math.sin(a)*580;if(this.brazier){c.drawImage(this.brazier,x-42,y-80,84,126);c.fillStyle='#ff9d4e';c.globalAlpha=.12+Math.sin(time*7+i)*.035;c.beginPath();c.arc(x,y-43,35,0,7);c.fill();c.globalAlpha=1;}}
  ...
  for(const f of g.fields){this.drawVfx(c,g.monster.id,f.x,f.y,Math.min(f.radius*1.45,480),f.kind==='vortex'?time*.55:0,f.kind==='vortex'?.32:.2);c.strokeStyle=g.identity.colors.power;c.lineWidth=3;c.setLineDash(f.kind==='meteor'?[10,8]:[]);c.beginPath();c.arc(f.x,f.y,f.radius,0,7);c.stroke();c.setLineDash([]);if(f.kind==='vortex'){c.beginPath();c.arc(f.x,f.y,f.radius*.6,time*3,time*3+5);c.stroke();}}
  ...
  const state=g.ultimateTime>0?'ultimate':g.player.invuln>0?'hurt':g.attack>g.monster.basic.interval*.65?'attack':g.moving?'move':'idle';if(!this.assets.drawGameplay(c,g.identity,cx,cy,time,state,g.monster.visual.scale)){if(this.assets.status(g.identity,'gameplay')==='failed')this.drawSovereign(c,cx,cy,time,g.player.invuln,g.monster.visual.scale,g.player.rage,g.identity.colors);else this.drawLoading(c,cx,cy,time,70*g.monster.visual.scale);}
  if(g.beam>0){c.save();c.translate(cx,cy);c.rotate(g.player.angle);if(this.vfx.has(g.monster.id)){for(let x=45;x<g.beamPower.radius;x+=75)this.drawVfx(c,g.monster.id,x,0,105,time*2+x*.02,.8);}else{c.fillStyle='#d6425380';c.fillRect(0,-35,g.beamPower.radius,70);c.fillStyle=g.identity.colors.power;c.fillRect(0,-17,g.beamPower.radius,34);c.fillStyle='#fff0d1';c.fillRect(0,-6,g.beamPower.radius,12);}c.restore();}
  ...
  for(const e of g.effects){const t=1-e.life/e.max;c.globalAlpha=1-t;c.lineWidth=4;
   ...
   else if(this.drawVfx(c,g.monster.id,e.x,e.y,Math.min(e.radius*2,520)*(e.kind==='devour'?1-t*.5:.65+t*.35),e.angle+time*.7,1)){}
