...
export class Renderer {
 canvas:HTMLCanvasElement;ctx:CanvasRenderingContext2D;width=0;height=0;scale=1;dpr=1;low=false;shake=true;autoLow=false;fxLevel=0;slowTime=0;fastTime=0;
 ...
 draw(g:Game|null,time:number){
  ...
  if(g.frenzyGuard>0){c.strokeStyle=g.identity.colors.power;c.lineWidth=3;c.beginPath();c.arc(cx,cy,65*g.monster.visual.scale,time*2,time*2+Math.PI*1.7);c.stroke();}
  ...
  if(g.beam>0){c.save();c.translate(cx,cy);c.rotate(g.player.angle);if(detail<2&&this.vfx.has(g.monster.id)){for(let x=45;x<g.beamPower.radius;x+=90)this.drawVfx(c,g.monster.id,x,0,105*g.releaseStats.beam,time*2+x*.02,.8);}else{c.fillStyle='#d6425380';c.fillRect(0,-30*g.releaseStats.beam,g.beamPower.radius,60*g.releaseStats.beam);c.fillStyle=g.identity.colors.power;c.fillRect(0,-17,g.beamPower.radius,34);c.fillStyle='#fff0d1';c.fillRect(0,-6,g.beamPower.radius,12);}c.restore();}
