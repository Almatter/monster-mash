...
export class Renderer {
 ...
 draw(g:Game|null,time:number){
  ...
  this.drawSovereign(c,cx,cy,time,g.player.invuln,1,g.player.rage);
  if(g.beam>0){c.save();c.translate(cx,cy);c.rotate(g.player.angle);c.fillStyle='#d6425380';c.fillRect(0,-35,680,70);c.fillStyle='#ffb77e';c.fillRect(0,-17,680,34);c.fillStyle='#fff0d1';c.fillRect(0,-6,680,12);c.restore();}
 ...
 drawSovereign(c:CanvasRenderingContext2D,x:number,y:number,time:number,hit:number,scale=1,rage=0){
  ...
  poly([-25,4,-38,49,-10,38,0,48,10,38,38,49,25,4],'#642d37');
  poly([-20,12,-24,38,-10,36,-5,14],'#867779');poly([20,12,24,38,10,36,5,14],'#867779');
  poly([-20,-22,-41,-13,-48,17,-34,28,-26,8,-18,1],hit>0?'#dfb89b':'#8b6c75');poly([20,-22,41,-13,48,17,34,28,26,8,18,1],hit>0?'#dfb89b':'#8b6c75');
  poly([-25,-24,0,-32,25,-24,28,4,0,29,-28,4],hit>0?'#e6b69a':'#806571');
  poly([-20,-18,0,-24,20,-18,15,5,0,19,-15,5],'#352732');
  poly([-22,-24,-43,-34,-36,-6,-23,-8],'#d2b590');poly([22,-24,43,-34,36,-6,23,-8],'#d2b590');
  poly([-13,-27,-27,-44,-29,-66,-15,-44,-5,-36],'#cfb18a');poly([13,-27,27,-44,29,-66,15,-44,5,-36],'#cfb18a');
  poly([-15,-39,0,-45,15,-39,13,-20,0,-10,-13,-20],'#a88b91');
  poly([-11,-32,-2,-29,-4,-24,-11,-26],'#ffe0a3');poly([11,-32,2,-29,4,-24,11,-26],'#ffe0a3');
  poly([-5,-1,0,-12,5,-1,0,11],'#ef7652');
  for(const side of [-1,1])for(let i=0;i<3;i++)poly([side*(35+i*4),15,side*(33+i*6),35,side*(40+i*3),21],'#e4c69c');
  c.restore();
 }
}
