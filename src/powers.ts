...
export const POWER_HANDLERS:Record<string,(game:Game,power:AbilityDef)=>void>={
 ...
 summon(g,p){for(let i=0;i<8+g.release*2;i++){const a=i/(8+g.release*2)*Math.PI*2;addServant(g,g.player.x+Math.cos(a)*70,g.player.y+Math.sin(a)*70,'summoned',Math.max(18,p.duration||0));}},
 ...
 meteor(g,p){if(g.fields.length<8)g.fields.push({kind:'meteor',x:g.targetPoint?.x??g.player.x+Math.cos(g.player.angle)*360,y:g.targetPoint?.y??g.player.y+Math.sin(g.player.angle)*360,life:p.duration||1.2,radius:p.radius,damage:p.damage,tick:0,kills:0});},
 vortex(g,p){if(g.fields.length<8)g.fields.push({kind:'vortex',x:g.targetPoint?.x??g.player.x+Math.cos(g.player.angle)*250,y:g.targetPoint?.y??g.player.y+Math.sin(g.player.angle)*250,life:p.duration||4,radius:p.radius,damage:p.damage,tick:0,kills:0});},
 lunge(g,p){g.dodgeInvuln=Math.max(g.dodgeInvuln,.16);g.dodgeDeflected=false;if(g.dash?.source==='lunge'){g.dash.remaining+=p.duration||.3;g.dash.angle=g.movementAngle;}else g.dash={remaining:p.duration||.3,speed:950*g.releaseStats.dash,damage:p.damage,kills:0,angle:g.movementAngle,radius:p.radius,source:'lunge'};},
 execute(g,p){let target:import('./simulation.ts').Enemy|null=null,best=Infinity;g.nearby(g.player.x,g.player.y,p.radius,e=>{const distance=Math.hypot(e.x-g.player.x,e.y-g.player.y);if(distance>p.radius)return;const rank=(e.kind==='titan'?0:e.kind==='elite'&&e.hp<e.maxHp?200:1000)+distance;if(rank<best){best=rank;target=e;}});if(target){const e=target as import('./simulation.ts').Enemy;g.effect(e.x,e.y,'execute',70,.5);const kills=g.damage(e,p.damage*g.powerScale()*(e.kind==='titan'?8:1)*(e.hp<e.maxHp*.5?2:1),'execute')?1:0;g.completeAttack(kills);}},
