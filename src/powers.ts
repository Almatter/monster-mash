...
export const POWER_HANDLERS:Record<string,(game:Game,power:AbilityDef)=>void>={
 ...
 lunge(g,p){g.dodgeInvuln=Math.max(g.dodgeInvuln,.16);g.dodgeDeflected=false;if(g.dash?.source==='lunge'){g.dash.remaining+=p.duration||.3;g.dash.angle=g.player.angle;}else g.dash={remaining:p.duration||.3,speed:950*g.releaseStats.dash,damage:p.damage,kills:0,angle:g.player.angle,radius:p.radius,source:'lunge'};},
