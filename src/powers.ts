...
export const POWER_HANDLERS:Record<string,(game:Game,power:AbilityDef)=>void>={
 dominate(g,p){dominate(g,p.radius,12);},
 curse(g,p){corrupt(g,p.radius,p.duration);g.area(g.player.x,g.player.y,p.radius,p.damage,'curse');},
 summon(g,p){for(let i=0;i<8;i++){const a=i*Math.PI/4;addServant(g,g.player.x+Math.cos(a)*70,g.player.y+Math.sin(a)*70,'summoned',p.duration);}},
 dominion(g,p){corrupt(g,p.radius);dominate(g,400,16);g.area(g.player.x,g.player.y,p.radius,p.damage,'ultimate');},
 ...
 lunge(g,p){g.dash={remaining:p.duration||.3,speed:950,damage:p.damage,kills:0,angle:g.player.angle,radius:p.radius,source:'lunge'};},
 ...
 frenzy(g,p){g.frenzy=p.duration||8;g.frenzyHealing=g.player.maxHp*.2;},
 shockwave(g,p){g.area(g.player.x,g.player.y,p.radius,p.damage,'shockwave',900);},
 devour(g,p){g.healBudget=g.player.maxHp*.25;g.area(g.player.x,g.player.y,p.radius,p.damage,'devour');g.player.rage=6;},
 ...
 charge(g,p){g.dash={remaining:p.duration||.65,speed:650,damage:p.damage,kills:0,angle:g.player.angle,radius:p.radius,source:'trample'};},
