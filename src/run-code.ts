...
export function validateRun(run:RunRecord){
 ...
 if(run.version===3){if(!['2026.10-v4-unbound',EVENT.rules].includes(run.rules)||!integer(run.release,4)||typeof run.build!=='string'||!/^[0-9a-f]{16}$/.test(run.build))throw Error('Invalid release or build metadata.');if(run.release!<releaseAt(Math.max(0,run.duration-.51))||run.release!>releaseAt(run.duration+.51))throw Error('Inconsistent release stage.');}
}
