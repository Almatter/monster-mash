...
export function createIdentity(value:Partial<Identity>={}):Identity{const monster=Object.hasOwn(MONSTERS,value.monsterId||'')?MONSTERS[value.monsterId!]:MONSTERS.sovereign;return {name:sanitizeName(value.name),title:sanitizeName(value.title,''),monsterId:monster.id,colors:paletteFor(monster,value.colors)};}
