...
const root='art-source/titan',out='public/assets/monsters/titan';
...
async function split(name,keyGreen){
 ...
 for(let y=0;y<info.height;y++)for(let x=0;x<info.width;x++){
  ...
  if(a<3)continue;
  const face=keyGreen?x>460&&x<820&&y>310&&y<540:x>430&&x<840&&y>230&&y<510;
  let channel='base',shade=0;
  if(r>g*1.42&&g>b*1.7&&g>45&&r>120){channel='power';shade=r/245;}
  else if(!face&&r>103&&g>75&&r>g*1.10&&g>b*1.35){channel='accent';shade=r/235;}
  else if(!face&&r>g*1.15&&r>b*1.12&&r>75&&b>27){channel='primary';shade=r/190;}
  else if(!face&&x>310&&x<1020&&y>380&&y<1000&&r>32&&r<108&&Math.abs(r-g)<27&&Math.abs(g-b)<27){channel='secondary';shade=r/105;}
  counts[channel]++;
...
await presentation('portrait',master,{left:320,top:95,width:690,height:690},512,512);
await presentation('cutin',master,{left:145,top:35,width:1020,height:900},1024,512);
for(const channel of channels){
 ...
 for(let row=0;row<5;row++)for(let frame=0;frame<[4,6,6,2,6][row];frame++){
  ...
  const sw=Math.round(220*(ultimate?1.06:attack?1.03:1)),sh=Math.round(195*(ultimate?1.06:attack?1.03:1));
